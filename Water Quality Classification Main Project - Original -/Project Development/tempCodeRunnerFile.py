...
def log_data():
    # Extract user input and prediction Result from the request
    ph = request.form['ph']
...
def register():
    if request.method == 'POST':
        ...
        if user is not None:
            conn.close()
            return "<script>alert('Error: User with the same security key already exists'); window.location.replace('/register');</script>"
        if user_by_name is not None:
            conn.close()
            return "<script>alert('Error: User with the same name already exists'); window.location.replace('/register');</script>"
